...
export async function compareArtifact({target,citeId,outPath="/tmp/shine-compare.png",lane="",brief="",brandLocked=false}) {
  ...
  const structure=assessStructure({facts:captured.facts,screen:row.screen,lane,brief,brandLocked,prior}); const visual=assessVisual({facts:captured.facts,row,tokens});
...
if(process.argv[1]===fileURLToPath(import.meta.url)){
  ...
  if(!target||!citeId){console.error("usage: node verify/compare.mjs <page|url> --cite <id> [--brief id --lane lane --brand-locked]");process.exit(2)}
  const result=await compareArtifact({target,citeId,outPath:opt("--out")||"/tmp/shine-compare.png",brief:opt("--brief"),lane:opt("--lane"),brandLocked:args.includes("--brand-locked")});
