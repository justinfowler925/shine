...
export async function capturePage(page) {
  const facts = await page.evaluate(() => {
    ...
    const surface = modal || document;
    const visible = (el) => { if (modal && !modal.contains(el)) return false; const r=el.getBoundingClientRect(),s=getComputedStyle(el); return r.width>0&&r.height>0&&s.display!=="none"&&s.visibility!=="hidden"; };
    ...
    const regionEls=[...document.querySelectorAll("header,nav,aside,main,section,footer,form,[role=form],dialog,table,[role=grid],[data-region]")].filter(visible);
    ...
    const interactions={search:!!surface.querySelector('input[type=search],[role=searchbox]'),sort:!!surface.querySelector('[data-sort],[aria-sort]'),page:!!surface.querySelector('[data-pagination],[aria-label*=pagination i]'),primary:!!surface.querySelector('[data-primary],button[type=submit]'),form:!!surface.querySelector("form,[role=form]"),rowAction:!!surface.querySelector('[data-row-action],tbody a,tbody button')};
