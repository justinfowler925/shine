...
export async function capturePage(page) {
  const facts = await page.evaluate(() => {
    ...
    return {title:document.title,bodyFont:body.fontFamily,bodySize:body.fontSize,bodyBg:body.backgroundColor,bodyColor:body.color,regions,controls,typography,palette,spacing,radii,interactions,tableCount:document.querySelectorAll("table,[role=grid]").length,rowCount:document.querySelectorAll("tbody tr,[role=row]").length,toolbar:!!document.querySelector('[data-toolbar],.toolbar,[role=search]'),voice:root.getAttribute("data-shine-voice")||"",family:root.getAttribute("data-dna-family")||"",signature:signature&&visible(signature)?{name:signature.getAttribute("data-shine-signature")||"",text:signature.textContent.trim().slice(0,120),box:box(signature)}:null,textChars:(document.body.innerText||"").length,viewport:{w:innerWidth,h:innerHeight}};
  });
