...
import {blocks,planBlocks,verifyReuse,sourceInventory} from '../integrations/blocks.mjs';
...
try{
 ...
 assert.equal(verifyReuse(temp,contract).status,'passed');
 mkdirSync(join(temp,'custom'),{recursive:true});writeFileSync(join(temp,'custom/widget.tsx'),'export function Widget(){return <div/>}');
 writeFileSync(join(temp,'shine-project.json'),JSON.stringify({sourceRoots:['custom'],tsconfig:'tsconfig.json'}));assert.equal(sourceInventory(temp)[0].path,'custom/widget.tsx');
 writeFileSync(join(temp,'shine-project.json'),JSON.stringify({sourceRoots:['../']}));assert.throws(()=>sourceInventory(temp),/inside the project/);rmSync(join(temp,'shine-project.json'));
