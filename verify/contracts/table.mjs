...
export async function evaluateDataGrids() {
  ...
  for (const grid of candidates) {
    const host = grid.closest("[data-grid],section,main") || grid.parentElement || document.body;
    ...
    const sortControl = grid.querySelector("thead button,[aria-sort] button,[data-sort],[role=columnheader] button");
    const sortSnapshot = () => `${rows()}|${queryAll("[aria-sort]").map((e) => e.getAttribute("aria-sort")).join()}`;
    const sort = await clickChanges(sortControl, sortSnapshot);
    const search = query('input[type="search"],[role="searchbox"],[data-filter-input]');
    let filter = false;
    if (search && visible(search)) {
      const before = rows();
      const old = search.value;
      search.value = search.getAttribute("data-shine-probe-value") || "__shine_no_match__";
    ...
    const sticky = Boolean(header && [header, ...header.querySelectorAll("th,[role=columnheader]")].some((e) => getComputedStyle(e).position === "sticky"));
    ...
    const bodyRows = queryAll("tbody tr,[role=row]").filter((r) => !r.closest("thead") && visible(r));
    ...
    results.push({
      ...
      resize: Boolean(grid.querySelector('[role="separator"],[data-column-resize],[data-resizable="true"]')),
      ...
      semantics: grid.tagName === "TABLE" || (grid.getAttribute("role") === "grid" && Boolean(grid.querySelector('[role="columnheader"]')) && Boolean(grid.querySelector('[role="gridcell"]'))),
      remoteMode: grid.hasAttribute("data-client-mode") || grid.hasAttribute("data-server-mode") || grid.hasAttribute("data-shine-data-mode"),
