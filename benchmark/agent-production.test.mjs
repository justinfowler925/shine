...
assert.match(buildPrompt({lane:"internal",brief:"fraud queue"}),/bounded design packet/);
for(const [name,mutate,needle] of [["runtime budget",x=>x.elapsedMs=180001,/runtime budget/],["input budget",x=>x.usage.input_tokens=250001,/input token budget/],["output budget",x=>x.usage.output_tokens=12001,/output token budget/]]){const seeded=structuredClone(good);mutate(seeded);assert.match(validateRunRecord(seeded).join(" "),needle,name)}
