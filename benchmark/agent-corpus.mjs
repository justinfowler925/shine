...
const manifest={version:1,harness:"Codex CLI native exec JSONL",model:"gpt-5.6-luna",reasoning:"medium",set:set||"full",concurrency:Number(process.env.SHINE_AGENT_CONCURRENCY||1),configurationVariable:"skillRoot only",baseline:{sha:gitSha(baseline),root:baseline},current:{sha:gitSha(current),root:current},historicalRuntime:"not reconstructed; causal control uses one current model/harness with only the skill tree varied",humanVotes:false};writeFileSync(join(out,"manifest.json"),JSON.stringify(manifest,null,2));
...
let next=0;const concurrency=Math.max(1,Math.min(2,Number(process.env.SHINE_AGENT_CONCURRENCY||1)));await Promise.all(Array.from({length:concurrency},async()=>{while(next<jobs.length){const job=jobs[next++];await execute(job)}}));
