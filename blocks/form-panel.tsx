...
export function FormPanel({ title, description, fields, initialValues, onSave, saveLabel = "Save changes" }: {
 ...
 return <section data-product-pattern="form-panel" aria-labelledby={id + "-heading"} className="min-w-0 rounded-lg border border-border bg-background">
  <header className="space-y-2 border-b border-border p-5"><h2 id={id + "-heading"} className="text-lg font-semibold">{title}</h2><p className="text-sm text-muted-foreground">{description}</p></header>
  <form noValidate onSubmit={async event => {
   ...
   <div className="space-y-5 p-5">{(error || Object.keys(errors).length > 0) && <div role="alert" ref={alert} tabIndex={-1} className="rounded-md border border-destructive p-3 text-sm"><p>{error || "Review these fields."}</p>{Object.entries(errors).map(([name, message]) => <a key={name} href={"#" + id + "-field-" + name} className="block underline">{message}</a>)}</div>}
   {fields.map(field => { const common = { id: id + "-field-" + field.name, name: field.name, disabled: busy, required: field.required, "aria-invalid": Boolean(errors[field.name]), "aria-describedby": field.help || errors[field.name] ? id + "-field-" + field.name + "-help" : undefined }; return <div key={field.name} className="space-y-2"><Label htmlFor={common.id}>{field.label}{field.required ? " (required)" : ""}</Label>
