...
export function createDesignPacket({job,lane="saas",project=process.cwd(),framework="",category="",mode="existing",productReference="",productReferenceName=""}){
 ...
 const diagnosis=reviewing?{required:true,reference:join(ROOT,"skill/references/diagnose.md"),command:`node ${join(ROOT,"core/diagnosis.mjs")} init --job ${JSON.stringify(job)} --category ${kind} --out shine-diagnosis.json`,verdicts:["defects","no-change"],instruction:"Name only defects you can evidence; one real defect is a complete diagnosis. If the surface is sound, record verdict no-change with verdictEvidence and every bucket in checked, and stop. Do not invent defects or inflate severity to satisfy a count."}:{required:false};
