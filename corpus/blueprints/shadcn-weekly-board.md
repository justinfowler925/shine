...
Regions, in order. Host: application shell. Density: comfortable. Paint: `tokens/voices/shadcn-zinc.css`.
...
- Prove with `verify/measure.mjs`, then `verify/usability.mjs` with a contract that records one outcome and moves between owners, then `verify/compare.mjs`.
